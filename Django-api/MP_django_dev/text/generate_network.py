...
def generate_sample_df(start, end, twitter_df):
    ...
    sample_df = twitter_df.loc[
        (twitter_df['created_at_CET'] >= start) &
        (twitter_df['created_at_CET'] <= end), ]

    return sample_df


# Extract 'user_mentions_id' and 'user_mentions_screen_name'
# from 'user_mentions'
...
def generate_network_df(sample_df):
    ...
    network_df = pd.DataFrame(columns=[
        'created_at_CET', 'place_full_name', 'user_screen_name', 'id', 'text',
        'retweet_count', 'favorite_count', 'is_retweet', 'is_favorite',
        'is_quote', 'is_reply', 'quoted_status_id_permalink',
        'in_reply_to_status_id', 'in_reply_to_user_id',
        'in_reply_to_screen_name', 'user_id', 'n_user_mentions',
        'user_mentions_id', 'user_mentions_screen_name', 'hashtags_name',
        'user_followers_count', 'user_following_count',
        'user_favourited_other_tweets_count', 'tweet_group', 'tweet_weight'])
    ...
    network_df['user_mentions_id'],
    network_df['user_mentions_screen_name'] = zip(
        *sample_df['user_mentions'].apply(get_user_mentions))
    ...
    # tweet weight
    network_df['tweet_weight'] = (
        sample_df['retweet_count'] + sample_df['favorite_count'])
...
# the function iterates over the dataframe, obtain the id and screen_name of
# the user that the author of that specific tweet reply or mention
# it returns the author of the specific tweet together with a list of users
# with whom the user interacted
def get_interactions(row):
    ...
    if row['user_mentions_id'] is not None:
        for i in range(len(row['user_mentions_id'])):
            interactions.add((
                row['user_mentions_id'][i],
                row['user_mentions_screen_name'][i]))
    ...
    for index, tweet in network_df.iterrows():
        ...
        for interaction in interactions:
            ...
            G.add_edge(
                ...
                int_id,
                tweet_id=tweet_id,
                tweet_group=tweet_group,
                edge_weight=tweet_weight,
                text=text)
...
def generate_events_df(file2, network_df):
    ...
    events_df['doc_list'] = events_df.doc_list.apply(
    ...
    events_df['weight'] = events_df.groupby(
        ['terms', 'user_id'])['id'].transform('count')
...
def generate_bipartite(events_df):
    ...
    B.add_weighted_edges_from([(
        row['terms'],
        row['user_id'],
        row['weight']) for idx, row in events_df.iterrows()])
