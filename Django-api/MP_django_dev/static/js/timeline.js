...
var svg_t,
		margin_t,// declares top focus size parameters
		margin2_t, // declares bottom context size parameters
		width_t,
		height_t,
		height2_t;
...
var parseDate = d3.timeParse("%Y-%m-%d"); //depends on the date format in the database
// var formatTime = d3.timeParse("%Y/%m/%d");

// declare x,y ranges in focus and context
var x,
		x2,
		y,
		y2;
...
function draw_timeline(term_type){
		...
		$.ajax({
				...
				success: function (data) {
						...
						// console.log("typeof " + jsonObject[1]["date"] + ": " + typeof(jsonObject[1]["pub_date"]));
						jsonObject = jsonObject.map(function(d) { return {date: parseDate(d.pub_date), count:d.count}; }) //reformat data 
						// var jsonObject = jQuery.parseJSON(data["response"][0]);//
						// console.log(jsonObject);
...
=======
// example data
var metricName   = "Tweet Amount";
var metricCount  = [1, 3, 1, 2, 1];
var metricMonths = ["2018-06-07", "2013-04-05", "2015-11-03", "2012-10-02", "2014-09-01"];
var optwidth        = 1080;
var optheight       = 256;
...
var margin	= {top: 20, right: 30, bottom: (optheight/2), left: 20},
    width	= optwidth - margin.left - margin.right,
    height	= optheight - margin.top - margin.bottom;

/* === Context chart === */

var margin_context = {top: (optheight/2)+20, right: 30, bottom: 20, left: 20},
