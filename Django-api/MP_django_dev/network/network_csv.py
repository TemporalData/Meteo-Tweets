...
def generate_sample_df(twitter_df):
    ...
    sample_df['date'] = sample_df['created_at_CET'].apply(lambda x: x.split(' ')[0])
    return sample_df
...
def generate_network_df(sample_df):
    ...
    network_df = pd.DataFrame(columns = ['date', 'place_full_name', 'latitude', 'longitude', 'user_screen_name', 'id', 'retweet_count', 'favorite_count', 
    ...
    same_columns = ['date', 'place_full_name', 'latitude', 'longitude', 'user_screen_name', 'id', 'retweet_count', 'favorite_count', 
                'is_retweet', 'is_favorite', 'is_quote', 'is_reply', 'quoted_status_id_permalink', 
                'in_reply_to_status_id', 'in_reply_to_user_id', 'in_reply_to_screen_name', 
