...
def load(file='partial.csv'):
    raw = pd.read_csv(file, engine='python', encoding='latin_1')
    # ,encoding='latin_1'
    # raw = raw.rename(columns={'Unnamed: 0': 'doc_no'})
    # # rename original indices
...
def clean(temp, isdate=False, start='2016-07-01', end='2016-07-08'):
    ...
    temp['text'] = temp['text'].apply(lambda x: x.rstrip('\\'))
    # remove distracting "\" at the end of strings
    temp['text'] = temp['text'].apply(lambda x: emoji.demojize(
...
#     source.data = dict(longitude=dev_merc_coord[0],
# latitude=dev_merc_coord[1])
...
# start=date(2015, 1, 1),
#                                  end=date(2018,9,6), value=(date(2015, 1, 1),
# date(2015, 12, 31)), step=1)
...
# other params(maybe by sliders):
# min_cluster_size, min_samples, eps, alpha, gen_min_span_tree
...
def word_to_list(data):
    ...
    word_list = []
    word_list += [
        word_tokenize(sentence) for sentence in data['text'].values.tolist()]
...
def find_weather_event(filename, lem_data, idx2doc):
    ...
    event_df = pd.DataFrame([{
        'terms': term,
        'doc_list': doc_list} for term, doc_list in weather_dict.items()])
    return event_df
...
#     weather_list = pd.read_csv(
# 'weather_dict.csv',engine='python').map(lambda x: x.lower()).values.tolist()
#     input_str =','.join(','.join(row) for row in data_list)
#     wordcloud = WordCloud(
# background_color='white', width = 1000, height= 600,
# max_words=word_no, contour_width=3)
...
def gensim_lda(word_list, n_topic=10, n_word=20):
    # Creatw the term dictionary of courpus,
    ...
    corpora.MmCorpus.serialize(
        os.path.join(lda_path+'corpus.mm'),
        doc_term_matrix)
    ...
    # model_data = pyLDAvis.gensim.prepare(
    # ldamodel, doc_term_matrix, dictionary)
    ...
    for i, line in enumerate(ldamodel[doc_term_matrix]):
        line = sorted(
            line, key=lambda x: (x[1]), reverse=True)
        # rank topic probabilities in descending order
        (topic_no, topic_prob) = line[0]
        # dominant topic with highest probability
        dominant_topic.append([
            int(topic_no),
            round(topic_prob, 4)])
        dominant_topic_df = pd.DataFrame(
            dominant_topic,
            columns=['dom_topic', 'dom_prob'])
...
def pipeline(start, end, filepath):
    ...
    cluster_model, clustered_data = do_hdbscan(
        data=target_data,
        ts=start,
        te=end)
    ...
    # desc_cluster_id = clustered_data.groupby(''cluster_id'').count()
    # .sort_values('doc_no',ascending=False).index# -1: noise
    # cluster_id = desc_cluster_id[0] if desc_cluster_id[0] != -1 else
    # desc_cluster_id[1]
...
def event_pipeline(eventname, filepath):
    ...
    doc_list = literal_eval(
        full_event_df[full_event_df.event == eventname]['doc_list'].values[0])
    ...
    ldamodel, dominant_topic_df, coherence, perplexity = gensim_lda(
        lem_data, n_topic=4, n_word=15)
